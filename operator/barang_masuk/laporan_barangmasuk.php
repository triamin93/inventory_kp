    ...
    if ($tgl_awal > $tgl_akhir) {
        ...
        $laporan = mysqli_query($conn, "SELECT * 
                                        FROM 
                                                barangmasuk 
                                        INNER JOIN 
                                                barang ON barangmasuk.id_barang = barang.id_barang
                                        INNER JOIN 
                                                supplier ON barangmasuk.id_supplier = supplier.id_supplier
                                        WHERE 
                                                tanggal_masuk 
                                        BETWEEN 
...
foreach ($laporan as $row) :
    ...
    $html .= '<tbody>
        <tr>
            ...
            <td>' . dateIndonesian($row["tanggal_masuk"]) . '</td>
            <td><center>' . $row["nama_barang"] . '</center></td>
            <td><center>' . $row["nama_supplier"] . '</center></td>
...
$mpdf->WriteHTML($html);
$mpdf->Output('laporan-barang-masuk.pdf', \Mpdf\Output\Destination::INLINE);
